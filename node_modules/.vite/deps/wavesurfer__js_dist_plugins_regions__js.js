//#region node_modules/wavesurfer.js/dist/plugins/regions.esm.js
var t = class {
	constructor() {
		this.listeners = {};
	}
	on(t, e, i) {
		if (this.listeners[t] || (this.listeners[t] = /* @__PURE__ */ new Set()), null == i ? void 0 : i.once) {
			const i = (...n) => {
				this.un(t, i), e(...n);
			};
...
function h(t, e = {}) {
	const { threshold: i = 3, mouseButton: n = 0, touchDelay: r = 100 } = e, o = s(null), h = /* @__PURE__ */ new Map(), a = matchMedia("(pointer: coarse)").matches;
	let d = () => {};
	const c = (e) => {
		if (e.button !== n) return;
		if (h.has(e.pointerId)) return;
		if (h.set(e.pointerId, e), h.size > 1) return;
		const s = e.pointerId;
		let l = e.clientX, c = e.clientY, u = !1;
		const p = Date.now(), { left: g, top: m } = t.getBoundingClientRect(), f = (t) => {
			if (t.pointerId !== s) return;
			if (t.defaultPrevented || h.size > 1) return;
			if (a && Date.now() - p < r) return;
			const e = t.clientX, n = t.clientY, d = e - l, v = n - c;
			(u || Math.abs(d) > i || Math.abs(v) > i) && (t.preventDefault(), t.stopPropagation(), u || (o.set({
				type: "start",
				x: l - g,
				y: c - m
			}), u = !0), o.set({
			...
			t.relatedTarget && t.relatedTarget !== document.documentElement || b(t);
		}, C = (t) => {
			u && (t.stopPropagation(), t.preventDefault());
		}, L = (t) => {
			t.defaultPrevented || h.size > 1 || u && t.preventDefault();
		};
		document.addEventListener("pointermove", f), document.addEventListener("pointerup", b), document.addEventListener("pointerout", E), document.addEventListener("pointercancel", E), document.addEventListener("touchmove", L, { passive: !1 }), document.addEventListener("click", C, { capture: !0 }), d = () => {
			document.removeEventListener("pointermove", f), document.removeEventListener("pointerup", b), document.removeEventListener("pointerout", E), document.removeEventListener("pointercancel", E), document.removeEventListener("touchmove", L), setTimeout((() => {
	...
	return {
		signal: o,
		cleanup: () => {
			d(), t.removeEventListener("pointerdown", c), h.clear(), l(o);
		}
	};
}
var a = class extends t {
	...
	addResizeHandles(t) {
		...
		}, t), o = h(i, { threshold: 1 }), l = h(s, { threshold: 1 }), a = r((() => {
	...
	toggleCursor(t) {
		var e;
		this.drag && !(null === (e = this.element) || void 0 === e) && e.style && (this.element.style.cursor = t ? "grabbing" : "grab");
	}
	initMouseEvents() {
		const { element: t } = this;
		if (!t) return;
		const e = o(t, "click"), i = o(t, "mouseenter"), n = o(t, "mouseleave"), s = o(t, "dblclick"), a = o(t, "pointerdown"), d = o(t, "pointerup"), c = e.subscribe(((t) => t && this.emit("click", t))), u = i.subscribe(((t) => t && this.emit("over", t))), p = n.subscribe(((t) => t && this.emit("leave", t))), v = s.subscribe(((t) => t && this.emit("dblclick", t))), g = a.subscribe(((t) => t && this.toggleCursor(!0))), m = d.subscribe(((t) => t && this.toggleCursor(!1)));
		this.subscriptions.push((() => {
			c(), u(), p(), v(), g(), m(), l(e), l(i), l(n), l(s), l(a), l(d);
		}));
		const f = h(t), b = r((() => {
...
var d = class d extends e {
	...
	avoidOverlapping(t) {
		t.content && !t.isRemoved && setTimeout((() => {
			...
			const e = t.content;
			e.style.marginTop = "0";
			const i = e.getBoundingClientRect(), n = this.regions.indexOf(t);
			if (n < 0) return;
			const s = this.regions.slice(0, n).filter(((t) => !t.isRemoved)).reduce(((e, n) => {
				if (n === t || !n.content) return e;
				const s = n.content.getBoundingClientRect();
				return i.left < s.right && s.left < i.right && e.push(s), e;
			}), []).sort(((t, e) => t.top - e.top)).reduce(((t, e) => {
				const n = i.top + t, s = n + i.height;
				return n < e.bottom && e.top < s ? e.bottom - i.top + 2 : t;
			}), 0);
			e.style.marginTop = `${s}px`;
		}), 10);
	}
	avoidOverlappingAll() {
		this.regions.forEach(((t) => this.avoidOverlapping(t)));
	}
	adjustScroll(t) {
		var e, i;
		if (!t.element) return;
	...
	virtualAppend(t, e, i) {
		...
		setTimeout((() => {
			...
			n();
			const e = this.wavesurfer.on("scroll", n), i = this.wavesurfer.on("zoom", n), s = this.wavesurfer.on("resize", n), r = t.on("render", n), o = [
				e,
				i,
				s,
				r
			];
			this.subscriptions.push(...o), t.once("remove", (() => {
				e(), i(), s(), r(), this.subscriptions = this.subscriptions.filter(((t) => !o.includes(t)));
			}));
		}), 0);
	}
	saveRegion(t) {
		...
		const e = [
			...
			t.on("update-end", ((e) => {
				this.avoidOverlappingAll(), this.emit("region-updated", t, e);
			})),
			t.on("play", ((e) => {
				var i;
			...
			t.once("remove", (() => {
				e.forEach(((t) => t())), this.subscriptions = this.subscriptions.filter(((t) => !e.includes(t))), this.regions = this.regions.filter(((e) => e !== t)), this.emit("region-removed", t);
			}))
		];
	...
	addRegion(t) {
		var e, i;
		if (!this.wavesurfer) throw Error("WaveSurfer is not initialized");
		const n = this.wavesurfer.getDuration(), r = new a(t, n, null === (i = null === (e = this.wavesurfer) || void 0 === e ? void 0 : e.getDecodedData()) || void 0 === i ? void 0 : i.numberOfChannels);
		if (this.emit("region-initialized", r), n) this.saveRegion(r);
		else {
			const t = this.wavesurfer.once("ready", ((e) => {
				r._setTotalDuration(e), this.saveRegion(r), this.subscriptions = this.subscriptions.filter(((e) => e !== t));
			}));
